...
module Actions =
    ...
    let addTurnToTurnHistory round turn =
        { round with TurnHistory = turn::round.TurnHistory }
    ...
    let addPoint player =
        { player with Points = player.Points + 1 }
    ...
    let turn _match playerOneCard playerTwoCard =
        ...
        let turn = createTurn playerOneMove playerTwoMove (TurnWinner winner)

        let updatedMatchWithTurn = addTurnToRoundHistory _match turn

        match winner with
        | Some winner when winner = playerOne
            -> printf "PlayerOne winner!"
        | Some winner when winner = playerTwo 
            -> printf "PlayerTwo winner!"
        | _ -> printf "Draw"
