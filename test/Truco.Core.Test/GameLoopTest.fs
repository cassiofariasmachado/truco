...
module GameLoopTest =
    ...
    let createTestTurn playerOneName playerTwoName winnerName =
        ...
        let winner =
            if winnerName = playerOneName then Some playerOne
            elif winnerName = playerTwoName then Some playerTwo
            else None

        createTurn (createPlayerMove playerOne (Ace, Sword)) (createPlayerMove playerTwo (Ace, Club)) winner
    ...
    let ``checkMatchWinner should return player one when they reach target score`` () =
        ...
        let matchWithPoints =
            { _match with
                PlayerOne = { _match.PlayerOne with Points = 12 } }
